...
const EndorsementAllonge: React.FC<EndorsementAllongeProps> = ({ notify }) => {
  const { instrumentImage } = useAppContext();
  ...
  const handleGenerate = async () => {
    ...
    setLegalRationale('');
    try {
      const { allongeText, legalRationale } = await generateAllonge(formData);
      setGeneratedDoc(allongeText || '');
      setLegalRationale(legalRationale || '');
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <div className="flex-1 flex flex-col gap-4 h-[600px] lg:h-auto">
        <div className="flex-1 bg-slate-200 rounded-lg p-4 lg:p-8 overflow-hidden relative border border-slate-300 shadow-inner">
            ...
            <div className="flex gap-2">
              {instrumentImage && (
                <button 
                  ...
                  className={`p-1.5 rounded text-slate-700 flex items-center gap-1.5 text-xs ${showPreview ? 'bg-slate-400' : 'hover:bg-slate-400'}`}
                  title="Preview on Instrument"
                >
          ...
          <div className="mt-8 flex-1 overflow-y-auto flex items-center justify-center">
            {showPreview && instrumentImage ? (
              <div className="relative w-full h-full">
                <img src={instrumentImage} alt="Instrument" className="w-full h-full object-contain" />
                {generatedDoc && (
                  <div className="absolute top-4 left-4 right-4 bg-white/80 backdrop-blur-sm p-4 border border-gray-400 shadow-lg rounded-md">
                    <h4 className="text-xs font-bold text-gray-800 border-b pb-1 mb-2">ENDORSEMENT ALLONGE (PREVIEW)</h4>
                    <div className="whitespace-pre-wrap text-xs text-black font-mono leading-snug">
                      {generatedDoc}
                    </div>
                    <div className="mt-4 pt-4 border-t-2 border-slate-800 flex justify-between items-end">
                      <div>
                        <div className="h-px w-32 bg-black mb-1"></div>
                        <p className="text-[8px] uppercase">Authorized Signature</p>
                      </div>
                    </div>
                  </div>
                )}
        ...
        {legalRationale && (
          <div className="bg-slate-900 rounded-xl border border-slate-800 p-4 animate-in fade-in">
            <div className="flex justify-between items-center mb-2">
              ...
              <button 
                onClick={() => {
                  navigator.clipboard.writeText(legalRationale);
                  notify?.('info', 'Rationale Copied');
                }}
